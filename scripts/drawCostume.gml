...
var costume = argument[0],
    ...
    _x = floor((argument[3] * global.screenScale) + .5) / global.screenScale,
    _y = floor((argument[4] * global.screenScale) + .5) / global.screenScale,
    _xscale = argument[5],
    _yscale = argument[6],
    col,
    alpha;
...
for (var i = 0; i < 4; i += 1)
{
    draw_sprite_part_ext(costume, i,
        1 + (sheetX * (SquareSize + 3)),
        1 + (sheetY * (SquareSize + 3)), SquareSize,
        SquareSize, _x - (24 * _xscale),
        _y - (20 * _yscale), _xscale, _yscale,
        col[i], alpha[i]);
}
